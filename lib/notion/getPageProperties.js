...
export default async function getPageProperties(
  ...
  properties.sub_type = properties.sub_type?.[0] || ''
  ...
  // properties.subTypeItem =
  //   properties?.sub_type?.map(subType => {
  //     return {
  //       name: subType,
  //       color: subTypeOptions?.find(s => s.value === subType)?.color || 'gray'
  //     }
  //   }) || []
...
function mapProperties(properties) {
  ...
  if (properties?.type === BLOG.NOTION_PROPERTY_NAME.type_omnisdoctrina) {
    properties.type = 'OmniDoc'
  }
