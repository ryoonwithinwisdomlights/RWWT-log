...
const CommonHead = ({ meta, children }) => {
  ...
  if (meta) {
    url = `${url}/${meta.slug}`
    image = meta.image || '/images/rwwt_lemon.png'
  }
  ...
  const lang = BLOG.LANG.replace('-', '_') // Facebook OpenGraph want zh_CN This format can capture the language
  const category = meta?.category || BLOG.KEYWORDS || 'Software Technology' // section Mainly like category Such classification, Facebook Use this to capture link categories
